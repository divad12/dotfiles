...
Turn any folder of files into a navigable knowledge graph with community detection, an honest audit trail, and agent-readable outputs: `graphify-out/wiki/`, GraphRAG-ready JSON, and a plain-language `GRAPH_REPORT.md`. HTML visualization is opt-in.
...
/graphify <path> --no-viz                             # skip optional visualization, keep report + JSON + wiki
/graphify <path> --html                               # also generate graph.html visualization
...
/graphify <path> --wiki                               # explicit wiki generation (default for agent workflows and hooks)
...
### Step 6 - Generate Obsidian vault (opt-in), wiki, and optional HTML

**Generate wiki by default** for agent workflows and hooks. **Generate HTML only if `--html` was explicitly given**; do not auto-regenerate `graph.html` from commit hooks. **Obsidian vault only if `--obsidian` was explicitly given** — skip it otherwise, it generates one file per node.
...
If `--html` was given, generate the optional HTML graph. Do not run this block from commit hooks or default agent workflows:
...
### Step 6b - Wiki (default)

Run this step by default, including hook rebuilds. `--wiki` is accepted as an explicit reminder; do not require it for agent workflows.
...
Graph complete. Outputs in PATH_TO_DIR/graphify-out/
  ...
  wiki/                 - agent-crawlable wiki (index.md entry point)
  graph.html            - optional HTML visualization (only if --html was given)
...
After every `git commit`, the hook detects which code files changed (via `git diff HEAD~1`), re-runs AST extraction on those files, and rebuilds `graph.json`, `GRAPH_REPORT.md`, and `wiki/`. Hooks must skip `graph.html`; generate HTML only when the user explicitly runs `/graphify --html`. Doc/image changes are ignored by the hook - run `/graphify --update` manually for those.
