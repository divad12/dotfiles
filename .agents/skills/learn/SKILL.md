...
For bugs that touch a shared data shape, API contract, hook signature, cache structure, or cross-module type: ask explicitly whether a shared contract changed. Name the contract, enumerate the consumer classes most likely affected (UI components, API callers, tests, fixtures, docs), and check whether sibling consumers carry the same stale assumption. A contract-level principle outlasts a single-field fix and helps future sessions find related bugs before they surface as user-facing failures.
