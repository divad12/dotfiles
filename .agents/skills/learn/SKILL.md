...
**Contract-level trigger:** when a bug appears, ask whether a shared contract changed — a type shape, API response, hook return, cache key, schema field, or protocol assumption. If yes: name the contract, list the consumer classes (DB selects, types, rendering surfaces, fixtures, docs), and climb to the highest actionable principle that would catch sibling failures across all consumers. A principle is most useful when it names the contract and points at a mechanical guardrail (schema scan, type check, shared helper, propagation test) that would surface the next sibling before it ships. Don't stop at the symptom; one contract change can hide several near-identical bugs in adjacent consumers.
