...
When the evidence involves a shared contract — an API response shape, TypeScript interface, Prisma schema field, hook return type, or cache key structure — climb to the contract level: name the contract, enumerate its consumer classes (UI hooks, API routes, test fixtures, seed scripts, plan stubs), and extract the principle that covers the whole class, not just the triggering instance. A contract-level principle is actionable when it helps find sibling failures across every consumer, not merely when it describes the incident that just happened.
