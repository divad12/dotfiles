...
For contract-level bugs, go further: check whether a shared contract changed (hook return shape, API payload, database schema field, cache key format, shared type). Name the contract, list its consumer classes (models, hooks, fixtures, docs, tests), and climb to the highest actionable principle — typically "when shared contract X changes, every consumer needs updating." A principle at this level is more useful than a specific symptom because it helps future agents proactively find sibling failures across the codebase.
