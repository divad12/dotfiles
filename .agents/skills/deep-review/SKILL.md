...
**Step 4: Extract explicit de-scopes.** While re-reading AGENTS.md/CLAUDE.md, note any capability the project has explicitly said it does not need yet — phrases like "No X yet", "X is not needed", "out of scope", "single-user only", "no mobile", "no i18n", "no RTL", "no a11y work yet". Record these as the **inadmissible list** for Step 3. Findings from any reviewer that would add or enforce a de-scoped capability are inadmissible regardless of their severity.
...
1. **Filter inadmissible findings.** Before deduplication, drop any finding that targets a capability the project has explicitly de-scoped (the inadmissible list built in Review 1.5 Step 4). These are not bugs, easy wins, or deferrals — they are out of scope for this project by design. If any inadmissible finding was auto-generated by the independent reviewer or UI subagent, note in the summary that it was dropped and why (e.g., "ARIA finding dropped — AGENTS.md:88 says no a11y work yet").
2. **Deduplicate** - if multiple reviews flag the same issue, mention it once and note that multiple reviewers caught it (higher confidence).
