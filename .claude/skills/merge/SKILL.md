...
4. **Verify** both branches point to the same commit:
   ```bash
...
5. **Report** the result: which commits were added, any conflicts resolved, final commit hash.
...
- **Local only.** All operations use local `main`, not `origin/main`. Do not `git fetch` or `git pull`. Do not push unless the user explicitly asks.
