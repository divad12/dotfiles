...
- When compressing instructions, keep rationale for rules that require judgment. Rationale tells the model how to generalize the rule correctly; without it, the model satisfies only the surface form. One compact sentence is enough — remove only redundant restatements.
- For safety-critical or correctness-critical rules, keep compact failure-mode rationale alongside the imperative: what goes wrong, what thought pattern is the stop signal, and what alternative action is allowed.
